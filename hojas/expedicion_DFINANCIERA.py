...
PLANTILLAS = {
    "NORMAL": "hojas/plantillas/TITULO_DFINANCIERA_NORMAL.docx"
}
...
def _is_yes(s: pd.Series) -> pd.Series:
    """True donde el valor equivale a 'sí' (robusto a variantes)."""
    ss = s.astype(str).str.strip().str.lower()
    ss = ss.str.replace("í", "i")  # normaliza 'sí' -> 'si'
    return ss.isin({"si", "yes", "true", "1", "verdadero"})

def run(df: pd.DataFrame):
    st.header("📄 Expedición título - Dirección Financiera")
    ...
    # ✅ Excluir los registros ya entregados
    ...
    if df.empty:
        st.success("✅ No hay títulos pendientes de Dirección Financiera (todos entregados).")
        return
